...
class Trainer():
    ...
    def start_train(self):
        try:
            ...
            print('ERROR in trainig loop or eval/save model.')
            raise
    ...
    def train_one_epoch(self, epoch):
        try:
            ...
            for step, batch_data in pbar:
                ...
                # Get statistics
                TP, FP, FN = self.get_statistics(self.model.predict(logits.detach()), labels,
                                                 TP, FP, FN)
                ...
                if step % 2 == 0:
                    ...
                    # Logging Loss
                    write_tbloss(self.tblogger, loss.detach().cpu(), (epoch * self.max_stepnum) + step)

                # Logging Acc
                write_tbacc(self.tblogger, TP / (TP + FP), epoch, 'train')
                write_tbPR(self.tblogger, TP, FP, FN, epoch, 'train')

        except Exception as _:
            print('ERRPR in training steps')
            raise
